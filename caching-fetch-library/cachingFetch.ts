...
// Global cache object for storing preloaded data
const globalCache: { [key: string]: unknown } = {};
...
export const useCachingFetch: UseCachingFetch = (url) => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(!cache.current[url]);
  const [data, setData] = useState<unknown>(cache.current[url] || null);
...
export const preloadCachingFetch = async (url: string): Promise<void> => {
  try {
    ...
    console.error('Preloading fetch failed:', error);
    throw error;
  }
};
...
export const serializeCache = (): string => {
  return JSON.stringify(globalCache);
};

export const initializeCache = (serializedCache: string): void => {
  const parsedCache = JSON.parse(serializedCache);
  Object.assign(globalCache, parsedCache);
};
